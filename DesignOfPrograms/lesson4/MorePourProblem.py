...
def more_pour_problem(capacities, goal, start=None):
    ...
    return shortest_path_search(start, capacities, successors, is_goal)


def replace(squence, i, val):
    s = list(squence)
    s[i] = val
    return type(squence)(s)
...
def successors(state, capacities):
    ...
    for i in range(len(capacities)):
        ...
        for j in range(len(capacities)):
            amount = min(state[i], capacities[j] - state[i])
            state2 = replace(state, i, state[amount] - amount)
...
def shortest_path_search(start, capacities, successors, is_goal):
...
print(successors((0, 0, 0, 0), (1, 2, 4, 8)))
